	...
	public function getTrack($routes, $uri)
	{
		foreach ($routes as $route) {
			...
			if (preg_match($pattern, $uri, $params)) {
				...
				// Отладка: посмотрите тип и значение перед вызовом intval()
				if (isset($params['test_id']) && !is_numeric($params['test_id'])) { // Проверка, является ли строкой или числом
					throw new \Exception("Некорректный test_id: " . print_r($params['test_id'], true));
				}
